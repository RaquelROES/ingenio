(function (blink) {
	...
	ModernStyle.prototype = {
		...
		bodyClassName: 'content_type_clase_ingenio',
		extraPlugins: ['image2'],
		ckEditorStyles: {
			name: 'ingenio',
			styles: [
				...
				{ name: 'Caja verde', type: 'widget', widget: 'blink_box', attributes: { 'class': 'box-1' } },
	...
	blink.theme.styles.ingenio = ModernStyle;
...
$(document).ready(function () {
	...
	var src_logo = $('.content_type_clase_ingenio').find('.logo_slide').attr('logo_dominio');
	if (typeof(src_logo) != 'undefined' && src_logo && src_logo != '' && src_logo.indexOf('gif1x1.gif') == -1) {
		$('.content_type_clase_ingenio').find('.logo-publisher').css('background-image', "url('"+src_logo+"')");
	}
});
